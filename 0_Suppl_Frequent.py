...
# ─────────────────────────────────────────────────────
# 4행×3열 = 4 users × 3 weeks (주신 순서 그대로 row-major)
# ─────────────────────────────────────────────────────
PANEL_SPECS_12 = [
    # User 1
    {"base_key": "bms_01241228055_2023-05", "start": "8",    "end": "15"},
    {"base_key": "bms_01241228055_2023-07", "start": "4",    "end": "11"},
    {"base_key": "bms_01241228055_2023-08", "start": "22",   "end": "29"},

    # User 2
    {"base_key": "bms_01241248924_2023-03", "start": "15",   "end": "22"},
    {"base_key": "bms_01241248924_2023-05", "start": "15",   "end": "22"},
    {"base_key": "bms_01241248924_2023-06", "start": "15",   "end": "22"},

    # User 3
    {"base_key": "bms_01241228095_2023-10", "start": "8",    "end": "15"},
    {"base_key": "bms_01241228095_2023-03", "start": "22",   "end": "29"},
    {"base_key": "bms_01241228095_2023-12", "start": "2:18", "end": "9:18"},

    # User 4
    {"base_key": "bms_01241228030_2023-06", "start": "15",   "end": "22"},
    {"base_key": "bms_01241228030_2023-10", "start": "25",   "end": "1"},   # 25일부터 "월말까지" 규칙
    {"base_key": "bms_01241228030_2024-01", "start": "1",    "end": "7"},
]
...
def parse_start_end_from_tokens(base_key: str, start_tok: str, end_tok: str) -> Tuple[pd.Timestamp, pd.Timestamp]:
    ...
    규칙:
      ...
      - end_day < start_day 이면 "월말까지"로 해석 (사용자 지정: 25,1)
...
# ─────────────────────────────────────────────────────
# ★ 변경사항: R 파일 접미사 허용
#  - 우선 {base_key}_R.csv 를 찾고,
#  - 없으면 호환을 위해 {base_key}_CR.csv 도 찾아봄
# ─────────────────────────────────────────────────────
def build_month_paths(vehicle_model: str, base_key: str) -> Tuple[str, str]:
    """
    파일명:
      - R(=non DFC): {base_key}_R.csv (주 사용)
                 (호환) {base_key}_CR.csv (fallback)
      - DFC       : {base_key}_DFC.csv
    """
    ...
    r_path_fallback = os.path.join(r_dir, f"{base_key}_CR.csv")  # 과거 산출물 호환
...
# ─────────────────────────────────────────────────────
def make_suppl_4users_3weeks_fullwidth():
    # cluster csv는 "있으면 쓰고, 없어도 돌아가게" 처리
    cl = None
    ...
    fig, axes = plt.subplots(
        nrows=4, ncols=3,
        figsize=(7.24, 5.10),  # full width (180 mm 근사)
        dpi=300,
    ...
    # sharex=True라서 숨겨진 y tick label을 모든 패널에서 강제로 다시 켬
    for ax in axes[:len(PANEL_SPECS_12)]:
        ax.tick_params(axis="y", labelleft=True)

    fig.tight_layout(rect=[0.04, 0.03, 0.995, 0.99])

    out_base = os.path.join(BASE_OUT, "Suppl_Fig_frequent_SCI")
...
if __name__ == "__main__":
    make_suppl_4users_3weeks_fullwidth()
