...
Pipeline:
  non-DFC CSV
    ...
    -> event labeling: Charg_Full, Charg_Partial, R_FC, R_other
    -> DFC synthesis
    -> *_DFC.csv + dfc_features_summary.csv

Label convention:
  Charg_Full      : full-charge segment
  Charg_Partial   : partial-charge segment
  R_FC            : post-full-charge rest / full-charge dwell segment
  R_other         : rest segments not assigned to Charg_Full, Charg_Partial, or R_FC

Notes:
  - Final *_DFC.csv outputs do not include parsing/label columns (charging, rest, Charg_Full, Charg_Partial, R_FC, R_other).
"""
...
# -----------------------------------------------------------------------------
# You can run this script without command-line arguments by filling these paths
# and pressing Run in PyCharm/VS Code. Command-line arguments still work and
# override this block when provided.
INPUT_PATH = r""       # CSV file path or folder path containing non-DFC CSV files
OUTPUT_DIR = r""       # Folder where *_DFC.csv files will be saved
SUMMARY_CSV = r""      # Optional. Leave blank to save dfc_features_summary.csv in OUTPUT_DIR
CSV_PATTERN = "*.csv"
WORKERS = 1
OVERWRITE = True

LABEL_FULL_CHARGE = "Charg_Full"
LABEL_PARTIAL_CHARGE = "Charg_Partial"
LABEL_FC = "R_FC"
LABEL_OTHER_REST = "R_other"

OUTPUT_DROP_COLUMNS = [
    "charging",
    "rest",
    LABEL_FULL_CHARGE,
    LABEL_PARTIAL_CHARGE,
    LABEL_FC,
    LABEL_OTHER_REST,
]
...
class DFCConfig:
    ...
    departure_buffer_minutes: float = 35.0
    fullcharge_parking_mode: bool = True
    merge_gap_minutes: float = 30.0
    min_other_hours_for_merge: Optional[float] = 7.0
...
def parse_charging(data: pd.DataFrame) -> pd.DataFrame:
    ...
    idx_s = pd.Series(data.index, index=data.index)
    g_first = idx_s.groupby(data["_chg_grp"], sort=False).first()
    g_last = idx_s.groupby(data["_chg_grp"], sort=False).last()
    ...
    idx_s2 = pd.Series(data.index, index=data.index)
    g2_first = idx_s2.groupby(data["_chg_grp2"], sort=False).first()
    g2_last = idx_s2.groupby(data["_chg_grp2"], sort=False).last()
...
def parse_rest(data: pd.DataFrame) -> pd.DataFrame:
    ...
    if abnormal.any():
        ...
        g_pc_first = g["pack_current"].first()
        g_val = (g_pc_first >= 0) & (g_pc_first <= 1)
        idx_s = pd.Series(data.index, index=data.index)
        g_first = idx_s.groupby(data["_ab_grp"], sort=False).first()
        g_last = idx_s.groupby(data["_ab_grp"], sort=False).last()
    ...
    g_val = g["rest"].first()
    idx_s = pd.Series(data.index, index=data.index)
    g_first = idx_s.groupby(data["_r_grp"], sort=False).first()
    g_last = idx_s.groupby(data["_r_grp"], sort=False).last()
...
def label_other_rest_and_merge(data: pd.DataFrame, config: DFCConfig) -> pd.DataFrame:
    """Label non-full-charge rest as R_other, optionally merge move-and-park behavior into R_FC."""
    data = data.copy()
    ...
    min_rest_td = None
    if config.min_other_hours_for_merge is not None and config.min_other_hours_for_merge > 0:
        min_rest_td = pd.Timedelta(hours=config.min_other_hours_for_merge)
    ...
    # Merge first R_other block after R_FC when gap is small and duration condition passes.
...
def apply_dfc(data: pd.DataFrame, config: DFCConfig, collect_stats: bool = True) -> tuple[pd.DataFrame, list[dict], dict]:
    ...
    events: list[dict] = []
    margin = pd.Timedelta(minutes=config.departure_buffer_minutes)
    ...
    # Parsing and event-label columns are always removed from the output.
    data = data.drop(columns=OUTPUT_DROP_COLUMNS, errors="ignore")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--no-fullcharge-parking-mode", action="store_true", help="Disable R_other to R_FC move-and-park merge")
    p.add_argument("--merge-gap-minutes", type=float, default=30.0)
    p.add_argument("--min-other-hours", type=float, default=7.0, help="Use <=0 to disable duration condition")
    p.add_argument("--full-soc-threshold", type=float, default=95.0)
    p.add_argument("--delay-soc-threshold", type=float, default=80.0)
    p.add_argument("--departure-buffer-minutes", type=float, default=35.0)
    return p
...
def main(argv: Optional[Iterable[str]] = None) -> None:
    ...
    # Direct-run mode: fill INPUT_PATH and OUTPUT_DIR above, then run this file
    # without command-line arguments. This is convenient in PyCharm/VS Code.
    if argv is None:
        import sys
        argv = sys.argv[1:]

    if len(list(argv)) == 0 and INPUT_PATH and OUTPUT_DIR:
        config = DFCConfig(
            departure_buffer_minutes=35.0,
            remove_label_columns=True,
        )
        process_path(
            input_path=INPUT_PATH,
            output_dir=OUTPUT_DIR,
            summary_csv_path=SUMMARY_CSV or None,
            config=config,
            pattern=CSV_PATTERN,
            workers=WORKERS,
    ...
    min_other = None if args.min_other_hours <= 0 else args.min_other_hours
    config = DFCConfig(
        ...
        min_other_hours_for_merge=min_other,
        remove_label_columns=True,
    )
